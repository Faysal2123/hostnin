        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {hostingPlans.map((plan) => (
            ...
            >
              ...
              <button
                onClick={() => router.push(`/${plan.title.toLowerCase().replace(/\s+/g, '-')}`)}
                className="w-full max-w-xs cursor-pointer border border-blue-100 text-blue-600 bg-white py-2 rounded-xl font-semibold hover:bg-blue-50 hover:border-blue-400 hover:text-black group-hover:bg-gradient-to-r group-hover:bg-white  group-hover:text-black group-hover:border-white transition-all duration-200 flex items-center justify-center gap-2"
              >
                {plan.buttonText}
                <span aria-hidden className="ml-1 group-hover:text-black transition-colors duration-200">&rarr;</span>
              </button>
            </div>
          ))}
        </div>
